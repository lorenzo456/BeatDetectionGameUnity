...
public class testBeat : MonoBehaviour {
    ...
    void Update()
    {
        ...
        float[] channelRight = spectrum;//song.GetComponent<AudioSource>().GetSpectrumData(1024, 1, FFTWindow.Hamming);
        float[] channelLeft = spectrum;// song.GetComponent<AudioSource>().GetSpectrumData(1024, 2, FFTWindow.Hamming);
